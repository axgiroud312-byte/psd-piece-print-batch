...
function groupRecord(
  ...
  return {
    ...
    output:
      state === "completed"
        ? { location: `runs/ui/${groupName}`, artifacts: [] }
        : undefined,
  };
}
...
function batchRecord(
  ...
  status: BatchRunRecord["status"] = "completed-with-errors",
): BatchRunRecord {
  return {
    schemaVersion: 1,
    runId: "ui-batch-001",
...
): NonNullable<PersistedOperatorWorkspace["latestRun"]> {
...
class FakeOperatorServices implements OperatorServices {
  ...
  loadFailure?: string;
  recovery = {
    records: [] as BatchRunRecord[],
    failures: [] as Array<{ source: string; message: string }>,
  };
  selectedTemplateJson: string | null = JSON.stringify(
    samplePreflightPayload.template,
    null,
    2,
  );
  selectedGroups: InputGroupSnapshot[] | null = structuredClone(
    samplePreflightPayload.groups,
  );
  startCalls = 0;
  retryCalls = 0;
  recoverCalls: string[] = [];
  ...
  retryResult = batchRecord(
    [groupRecord("款式001-蓝花", "completed")],
    "completed",
  );

  async loadWorkspace() {
    return {
      workspace: this.workspace ? structuredClone(this.workspace) : undefined,
  ...
  async selectAndVerifyMaster(template: TemplateConfig) {
    return {
      sourceRef: "verified-persistent-master",
      fingerprint: template.masterFingerprint,
    };
  }
  ...
  async runTrial(
    ...
    if (this.trialImplementation)
      return this.trialImplementation(input, options);
  ...
  async startBatch(
    ...
    if (this.startImplementation)
      return this.startImplementation(input, options);
    ...
    return batchRecord(
      input.groups.map((group) => groupRecord(group.name, "completed")),
      "completed",
    );
  }
  ...
  async retryBatch(
    ...
    this.retryCalls += 1;
    if (this.retryImplementation)
      return this.retryImplementation(runId, input, options);
  ...
  async recoverRun(runId: string) {
    ...
    const record = this.recovery.records.find(
  ...
  async confirmManualCleanup(runId: string, groupName: string) {
    ...
    if (this.cleanupImplementation)
      return this.cleanupImplementation(runId, groupName);
    return structuredClone(this.retryResult);
  }
}
...
async function preparePreflight(services: FakeOperatorServices): Promise<void> {
  ...
  await vi.waitFor(() =>
    expect(
      document.querySelector<HTMLTextAreaElement>(".template-editor")?.value,
    ).toContain(samplePreflightPayload.template.templateId),
  );
  click(".master-select");
  await vi.waitFor(() =>
    expect(document.body.textContent).toContain("母版内容指纹已核对"),
  );
  click(".template-save");
  await vi.waitFor(() =>
  ...
  click(".input-scan");
  await vi.waitFor(() =>
    expect(document.querySelector(".preflight-summary")?.textContent).toContain(
      "1 组可运行",
    ),
  );
...
async function prepareApprovedTrial(
  services: FakeOperatorServices,
): Promise<void> {
  await preparePreflight(services);
  click(".trial-run");
  await vi.waitFor(() =>
    expect(
      document.querySelector(".trial-result__status")?.textContent,
    ).toContain("试套事务已完成"),
  );
  click(".trial-approve");
  await vi.waitFor(() =>
    expect(document.body.textContent).toContain("正式批次已解锁"),
  );
...
describe("five-stage operator panel", () => {
  ...
  it("renders five stages from one workflow state and keeps production fail-closed by default", async () => {
    ...
    await mounted.ready;
    const buttons =
      document.querySelectorAll<HTMLButtonElement>("[data-stage]");
    ...
    expect(
      document.querySelector<HTMLButtonElement>(".batch-start")?.disabled,
    ).toBe(true);
  ...
  it("saves a template draft across navigation without persisting its session reference", async () => {
    ...
    await vi.waitFor(() =>
      expect(
        document.querySelector<HTMLTextAreaElement>(".template-editor")?.value,
      ).toContain("masterSourceRef"),
    );
    click(".template-save");
    await vi.waitFor(() => expect(services.workspace?.draft).toBeDefined());

    expect(services.workspace?.draft?.template).not.toHaveProperty(
      "masterSourceRef",
    );
    expect(JSON.stringify(services.workspace)).not.toContain(
      samplePreflightPayload.template.masterSourceRef,
    );
    expect(JSON.stringify(services.workspace)).not.toContain(
      "persistent-master",
    );
    await vi.waitFor(() =>
      expect(
        document.querySelector<HTMLButtonElement>('[data-stage="input"]')
          ?.disabled,
      ).toBe(false),
    );
    click('[data-stage="input"]');
    ...
    expect(
      document.querySelector<HTMLElement>('[data-stage="template"]')?.dataset
        .workflowStatus,
    ).toBe("completed");
  });
  ...
  it("keeps formal run locked until preflight, trial completion, and operator approval", async () => {
    ...
    expect(
      document.querySelector<HTMLElement>('[data-stage="preview"]')?.dataset
        .workflowStatus,
    ).toBe("completed");
  });
  ...
  it("does not allow a trial with unresolved cleanup to unlock production", async () => {
    ...
    click(".trial-run");
    await vi.waitFor(() =>
      expect(document.body.textContent).toContain("试套不能放行"),
    );
    expect(
      document.querySelector<HTMLButtonElement>(".trial-approve")?.disabled,
    ).toBe(true);
  ...
  it("revokes the prior approval while a replacement trial is running", async () => {
    ...
    services.trialImplementation = async (input) =>
      new Promise<GroupRunResult>((resolve) => {
    ...
    expect(
      document.querySelector<HTMLButtonElement>('[data-stage="run"]')?.disabled,
    ).toBe(true);
  ...
  it("invalidates an approved trial as soon as template rules change", async () => {
    ...
    click('[data-stage="template"]');
    const editor =
      document.querySelector<HTMLTextAreaElement>(".template-editor")!;
  ...
  it("updates visible workflow state immediately when an input editor becomes stale", async () => {
    ...
    const editor =
      document.querySelector<HTMLTextAreaElement>(".groups-editor")!;
    ...
    expect(
      document.querySelector<HTMLElement>('[data-stage="input"]')?.dataset
        .workflowStatus,
    ).toBe("current");
  });
  ...
  it("shows live group/stage progress and requests stop through the safe cancellation token", async () => {
    ...
    let capturedCancellation:
      Parameters<OperatorServices["startBatch"]>[1]["cancellation"] | undefined;
    services.startImplementation = async (input, options) => {
      ...
      return new Promise<BatchRunRecord>((resolve) => {
    ...
    click(".batch-start");
    await vi.waitFor(() =>
      expect(document.querySelector(".batch-progress")?.textContent).toContain(
        "替换印花素材",
      ),
    );
    expect(document.querySelector(".batch-progress")?.textContent).toContain(
    ...
    resolveBatch(
      batchRecord([groupRecord("款式001-蓝花", "interrupted")], "interrupted"),
    );
    await vi.waitFor(() =>
      expect(
        document.querySelector('[data-result-state="interrupted"]')
          ?.textContent,
      ).toContain("款式001-蓝花"),
    );
  });
  ...
  it("separates result states and blocks retry while a group still needs recovery", async () => {
    ...
    await mount(services);
    await prepareApprovedTrial(services);
    services.startImplementation = async () =>
      batchRecord([
    ...
    await vi.waitFor(() =>
      expect(
        document.querySelector('[data-result-state="failed"]')?.textContent,
      ).toContain("失败组"),
    );

    expect(
      document.querySelector('[data-result-state="completed"]')?.textContent,
    ).toContain("完成组");
    expect(
      document.querySelector('[data-result-state="interrupted"]')?.textContent,
    ).toContain("中断组");
    expect(
      document.querySelector('[data-result-state="review-required"]')
        ?.textContent,
    ).toContain("待确认组");
    expect(
      document.querySelector<HTMLButtonElement>(".batch-retry")?.disabled,
    ).toBe(true);
    expect(
      document.querySelector<HTMLButtonElement>(".batch-recover")?.disabled,
    ).toBe(false);
  });
  ...
  it("provides the explicit manual cleanup confirmation required by recovery", async () => {
    ...
    services.startImplementation = async () =>
      batchRecord([review], "interrupted");
    services.cleanupImplementation = async () =>
      batchRecord([groupRecord("人工清理组", "interrupted")], "interrupted");
    click('[data-stage="run"]');
    click(".batch-start");
    await vi.waitFor(() =>
      expect(document.querySelector(".cleanup-confirm")).not.toBeNull(),
    );

    click(".cleanup-confirm");
    await vi.waitFor(() =>
      expect(document.body.textContent).toContain("人工清理确认"),
    );
    expect(services.cleanupCalls).toEqual([
      { runId: "ui-batch-001", groupName: "人工清理组" },
    ]);
  });
  ...
  it("requires recovery before confirming cleanup for a newly completed group", async () => {
    ...
    services.startImplementation = async () =>
      batchRecord([completed], "interrupted");
    click('[data-stage="run"]');
    click(".batch-start");
    await vi.waitFor(() =>
      expect(document.querySelector(".batch-recover")).not.toBeNull(),
    );
  ...
  it("retries eligible failed groups and passes a cancellable run request", async () => {
    ...
    await prepareApprovedTrial(services);
    services.startImplementation = async () =>
      batchRecord([groupRecord("款式001-蓝花", "failed")]);
    let resolveRetry!: (record: BatchRunRecord) => void;
    let retryCancellation:
      Parameters<OperatorServices["retryBatch"]>[2]["cancellation"] | undefined;
    ...
    await vi.waitFor(() =>
      expect(document.body.textContent).toContain("重试后批次全部完成"),
    );
  ...
  it("restores saved rules and interrupted runs while requiring master reselection", async () => {
    const services = new FakeOperatorServices();
    const { masterSourceRef: _sessionReference, ...definition } =
      samplePreflightPayload.template;
    services.workspace = {
      schemaVersion: 1,
      draft: {
        schemaVersion: 1,
        template: definition,
        templateFingerprint: createTemplateFingerprint(
          samplePreflightPayload.template,
          "0.1.0",
        ),
        savedAt: now,
      },
      latestRun: persistedBatch(
        batchRecord([groupRecord("中断组", "interrupted")], "access-required"),
      ),
    };
    services.recovery.failures.push({
      source: "broken.json",
      message: "JSON 损坏",
    });
    await mount(services);
  ...
  it("reconciles raw running and queued crash records before showing recovery results", async () => {
    ...
    services.recoveryImplementation = async () =>
      batchRecord(
        [
          {
            ...groupRecord("提交中组", "interrupted"),
            interruptionReason: "crash",
          },
          {
            ...groupRecord("未开始组", "interrupted", 0),
  ...
  it("keeps recovered retry locked until its template and input snapshots are restored", async () => {
    const services = new FakeOperatorServices();
    services.recovery.records.push(
      batchRecord([groupRecord("待重试组", "failed")], "completed-with-errors"),
    );
    await mount(services);
  ...
  it("lets the operator switch between multiple recoverable runs", async () => {
    const services = new FakeOperatorServices();
    const older = batchRecord(
      [groupRecord("旧批次组", "interrupted")],
      "interrupted",
    );
    older.runId = "run-older";
    const newer = batchRecord(
      [groupRecord("新批次组", "interrupted")],
      "interrupted",
    );
  ...
  it("offers directory reselection before recovering an access-locked run", async () => {
    ...
    await vi.waitFor(() =>
      expect(document.body.textContent).toContain("目录访问已恢复"),
    );
  ...
  it("does not claim success when reselected directories still fail validation", async () => {
    const services = new FakeOperatorServices();
    const locked = batchRecord(
      [groupRecord("中断组", "interrupted")],
      "access-required",
    );
    services.workspace = {
      schemaVersion: 1,
      latestRun: persistedBatch(locked),
    };
    ...
    click(".access-reselect");
    await vi.waitFor(() =>
      expect(document.body.textContent).toContain("所选目录仍无法满足恢复要求"),
    );
  ...
  it("starts through the plugin entrypoint", async () => {
    ...
    await vi.waitFor(() =>
      expect(document.querySelectorAll("[data-stage]")).toHaveLength(5),
    );
  });
});
