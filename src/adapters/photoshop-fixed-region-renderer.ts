...
function documentById(
  ...
  const document = runtime.documents.find(
...
function uniqueLayer(
  ...
  if (matches.length !== 1)
    throw new Error(`输出可见性路径歧义：${[...path, name].join(" / ")}`);
  return matches[0];
}
...
function requireDocumentControl(
  control: ModalDocumentControl | undefined,
): ModalDocumentControl {
  if (!control)
    throw new Error("真实 Photoshop 输出必须在带自动关闭保护的 modal 中运行");
  return control;
}
...
function defaultRuntime(): PhotoshopRenderRuntime {
  ...
  return {
    ...
    createSolidBackground: async (document, color) => {
      ...
      if (!background?.move || !bottom)
        throw new Error("无法把输出底色放到图层底部");
      if (background.id !== bottom.id) {
        await background.move(
          bottom,
...
export class PhotoshopFixedRegionRenderer implements FixedRegionRenderer {
  ...
  private assertDocumentProfile(
    ...
    if (
      ...
      document.colorProfileName !== target.profile.icc.profile
    ) {
      throw new Error(
        `输出 ${target.fileName} 要求 ICC ${target.profile.icc.profile}，工作副本为 ${document.colorProfileName ?? "无"}`,
      );
    }
  }
    ...
    await withOwnedDocument(output, control, async () => {
      ...
      if (!editable) {
        ...
        if (target.profile.background.kind === "solid") {
          await this.runtime.createSolidBackground(
            output,
            target.profile.background.color,
          );
        }
      ...
      try {
        if (target.profile.format === "png") {
          ...
          await output.saveAs.jpg(
            entry,
            { quality: 12, embedColorProfile },
            true,
          );
        } else if (target.profile.format === "psd") {
          await output.saveAs.psd(
            entry,
            { embedColorProfile, maximizeCompatibility: true },
            true,
          );
        } else {
          await output.saveAs.psb(
            entry,
            { embedColorProfile, maximizeCompatibility: true },
            true,
          );
        ...
        if (
          cancellation.number === -128 ||
          /cancelled|canceled|取消/i.test(cancellation.message ?? "")
        )
          throw error;
        throw new BatchStoppingError(
          "photoshop-output-save-failed",
          error instanceof Error
            ? `Photoshop 保存 ${target.fileName} 失败：${error.message}`
            : `Photoshop 保存 ${target.fileName} 失败`,
        );
      }
    });
  }
  ...
  async inspect(
    ...
    const control = requireDocumentControl(documentControl);
    const document = await this.runtime.open(
      await this.storage.fileEntry(location),
    );
    return withOwnedDocument(document, control, async () => {
      ...
      return {
        ...
        colorMode:
          normalizedColorMode(document.mode) ??
          ("invalid" as OutputArtifactMetadata["colorMode"]),
        bitDepth:
          normalizedBitDepth(document.bitsPerChannel) ??
          (0 as OutputArtifactMetadata["bitDepth"]),
        iccProfile:
