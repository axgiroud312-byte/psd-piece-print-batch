...
import type { UxpOutputEntry, UxpOutputFile, UxpOutputFolder } from "./uxp-output-storage";
...
function isRecord(value: unknown): value is BatchRunRecord {
  ...
  const statuses = new Set(["running", "completed", "completed-with-errors", "interrupted", "access-required"]);
  const states = new Set(["queued", "running", "completed", "failed", "interrupted", "review-required"]);
  return (
...
export class UxpBatchRunStore implements BatchRunStore {
  ...
  private async entry(name: string): Promise<UxpOutputEntry | undefined> {
    ...
    return (await this.options.folder.getEntries()).find((entry) => entry.name.toLowerCase() === normalized);
  }

  private async read(file: UxpOutputFile): Promise<BatchRunRecord> {
    const data = await file.read({ format: this.options.binaryFormat });
    if (typeof data === "string") throw new Error(`运行记录无法按二进制读取：${file.name}`);
    let parsed: unknown;
    try {
  ...
  private async write(file: UxpOutputFile, record: BatchRunRecord): Promise<void> {
    const bytes = new TextEncoder().encode(JSON.stringify(record, null, 2));
    const written = await file.write(arrayBuffer(bytes), { format: this.options.binaryFormat });
    if (written !== bytes.byteLength) throw new Error(`运行记录写入不完整：${file.name}`);
  }

  async create(record: BatchRunRecord): Promise<void> {
    const name = this.fileName(record.runId);
    if (await this.entry(name)) throw new Error(`运行编号已存在：${record.runId}`);
    const temporary = await this.options.folder.createFile(`${name}.tmp`, { overwrite: true });
    await this.write(temporary, record);
    await temporary.moveTo(this.options.folder, { overwrite: false, newName: name });
  }
  ...
  async save(record: BatchRunRecord): Promise<void> {
    ...
    const temporary = await this.options.folder.createFile(temporaryName, { overwrite: true });
    await this.write(temporary, record);
    await temporary.moveTo(this.options.folder, { overwrite: true, newName: name });
  }
