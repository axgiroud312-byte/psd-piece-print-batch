...
export const workflowStages: WorkflowStage[] = [
  {
    ...
    label: "模板",
    title: "登记生产母版",
    description: "导入、校验并保存已经认可的母版结构和输出规则。",
  },
  {
    id: "input",
    number: "02",
    label: "输入",
    title: "检查素材分组",
    description: "扫描素材总文件夹，按固定名称规则确认每一组是否可运行。",
  },
  {
    id: "preview",
    number: "03",
    label: "预览",
    title: "先试套一组",
    description: "正式批量前必须完成一组试套并由操作员确认。",
  },
  {
    id: "run",
    number: "04",
    label: "运行",
    title: "串行处理批次",
    description: "逐组创建干净工作副本，并只在安全边界响应停止。",
  },
  {
    id: "results",
    number: "05",
    label: "结果",
    title: "复核并处理失败组",
    description: "分开查看完成、失败、中断和待确认项目，并只重试合格项目。",
  },
];
...
type BusyAction =
  ...
  | "access";
type WorkflowStatus =
  "locked" | "ready" | "current" | "completed" | "attention";
...
function trialIsCurrent(state: OperatorState): boolean {
  if (!state.trial?.approved || !state.template || state.templateDirty)
    return false;
  if (state.trial.templateFingerprint !== state.templateFingerprint)
    return false;
  const group = state.groups.find(
    (candidate) => candidate.name === state.trial?.groupName,
  );
  if (!group) return false;
  try {
    return (
      createTaskFingerprint(state.template, group, pluginVersion) ===
      state.trial.taskFingerprint
    );
  } catch {
    return false;
  }
...
function trialRunIsClean(state: OperatorState): boolean {
  return (
    state.trialRun?.status === "completed" &&
...
function startBlockReason(
  ...
  if (!state.template || state.templateDirty || !state.templateFingerprint)
    return "请先保存有效模板草稿。";
  if (state.masterAccessRequired)
    return "重启后母版访问需要重新选择并校验指纹。";
  if (!state.report || state.report.validGroupCount === 0)
    return "请先完成素材预检，并至少保留一组可运行素材。";
  if (!trialIsCurrent(state))
    return "当前模板、规则和素材组合尚未完成已确认试套。";
  return undefined;
}
...
function retryBlockReason(
  ...
  if (state.busy) return "请等待当前操作完成。";
  if (!services.capability.productionEnabled) return services.capability.advice;
  if (!state.template || state.templateDirty)
    return "请先恢复并保存此批次使用的模板规则。";
  if (state.masterAccessRequired)
    return "请先重新选择母版 PSD/PSB 并核对内容指纹。";
  if (!state.report)
    return "请重新选择素材总文件夹并完成预检，系统会核对每组组合指纹。";
  const available = new Set(validGroups(state).map((group) => group.name));
  const missing = state.batch?.groups.find(
    (group) =>
      (group.state === "failed" || group.state === "interrupted") &&
      !available.has(group.groupName),
  );
  if (missing)
    return `请重新扫描并通过预检：当前缺少 ${missing.groupName} 的有效素材快照。`;
  return undefined;
}

function failureAdvice(group: BatchGroupRecord): string | undefined {
  const code = group.failure?.code;
  if (code === "input-access-expired")
    return "重新选择素材总文件夹；系统将重新扫描并核对组合指纹。";
  if (code === "retry-fingerprint-changed")
    return "模板、素材或规则已经变化，请重新预检并完成新试套。";
  if (code === "photoshop-capability-unavailable")
    return "当前 Photoshop 版本或 M0 能力不匹配，正式生产保持锁定。";
  if (group.cleanupRequiresReview)
    return "请先确认 Photoshop 中插件创建的临时文档已经关闭。";
  if (group.cleanupWarning)
    return `临时资源清理未完成：${group.cleanupWarning}。请先执行恢复并核对输出。`;
  if (group.state === "review-required")
    return "系统无法安全判断提交或临时状态，请保留现有文件并人工复核。";
  return group.error;
}
...
function workflowStatus(
  ...
  if (stage === "preview") {
    if (!state.report?.validGroupCount || state.masterAccessRequired)
      return "locked";
    if (trialIsCurrent(state)) return "completed";
    if (state.trialRun && state.trialRun.status !== "completed")
      return "attention";
    return state.viewedStage === stage ? "current" : "ready";
  }
  if (stage === "run") {
    ...
    return startBlockReason(state, services)
      ? "locked"
      : state.viewedStage === stage
        ? "current"
        : "ready";
  }
  if (
    ...
    state.persistenceFailures.length > 0
  ) {
...
function renderPreflightReport(
  ...
  for (const mapping of report.templateSummary.mappings) {
    templateCard.appendChild(
      createElement("p", "template-result__mapping", mapping),
    );
  }
  for (const issue of report.templateIssues) {
    templateCard.appendChild(
      createElement(
        "p",
        `group-result__issue group-result__issue--${issue.severity}`,
        issue.message,
      ),
    );
  }
  ...
  for (const group of report.groups) {
    const card = createElement(
      "section",
      `group-result group-result--${group.status}`,
    );
    ...
    appendChildren(
      ...
      createElement(
        "span",
        "group-result__status",
        group.status === "valid" ? "可运行" : "已阻止",
      ),
    );
      ...
      createElement(
        ...
        group.assignments.length > 0
          ? group.assignments
              .map(
                (item) =>
                  `${item.entryId} / ${item.contentSourceId} ← ${item.fileName}`,
              )
              .join(" / ")
          : "没有可提交的素材映射",
      ),
...
function renderResultColumn(
  ...
  const section = createElement(
    "section",
    `result-column result-column--${stateName}`,
  );
  ...
  section.appendChild(
    createElement(
      ...
      `${groupStateLabels[stateName]} ${matching.length}`,
    ),
  );
  if (matching.length === 0)
    section.appendChild(createElement("p", "empty-state", "暂无"));
  for (const group of matching) {
    ...
    appendChildren(
      ...
      createElement(
        "span",
        "result-item__attempt",
        `尝试 ${group.attemptCount}`,
      ),
    );
    const advice = failureAdvice(group);
    if (advice)
      card.appendChild(createElement("p", "result-item__advice", advice));
    if (group.output)
      card.appendChild(
        createElement("p", "result-item__output", group.output.location),
      );
    if (
      ...
      const confirm = actionButton(
        "我已关闭插件临时文档",
        "secondary-action cleanup-confirm",
      );
      confirm.dataset.groupName = group.groupName;
      confirm.addEventListener("click", () => {
        onConfirmCleanup(group.groupName);
      });
...
export function mountApp(
  ...
  const workspace = (): PersistedOperatorWorkspace => {
    ...
    if (state.template && state.templateFingerprint && state.templateSavedAt) {
      const { masterSourceRef: _sessionReference, ...template } =
        state.template;
      draft = {
        schemaVersion: 1,
    ...
    return {
      ...
      latestRun: state.batch
        ? (() => {
            const { accessGrants: _persistentAccessGrants, ...record } =
              state.batch;
  ...
  const persist = async (): Promise<void> => {
    try {
      ...
      state.notice = {
        kind: "error",
        message: `无法保存操作状态：${errorMessage(error)}`,
      };
    }
  };
  ...
  const saveTemplate = async (): Promise<void> => {
    ...
    try {
      ...
      if (hasTemplateErrors(issues))
        throw new Error("模板仍有阻止运行的问题，请按下方提示修正");
      ...
      setNotice(
        "success",
        `模板 ${template.templateId} / ${template.version} 已保存`,
      );
      await persist();
    } catch (error) {
  ...
  const preflightInput = async (): Promise<void> => {
    ...
    try {
      ...
      state.groups = groups;
      state.report = report;
      state.trialGroupName = report.groups.find(
        (group) => group.status === "valid",
      )?.groupName;
  ...
  const runTrial = async (): Promise<void> => {
    ...
    const group = validGroups(state).find(
      (candidate) => candidate.name === state.trialGroupName,
    );
    if (!group) return;
    state.busy = "trial";
    state.trial = undefined;
      ...
      const result = await services.runTrial(
        { template: state.template, group },
        {
          onEvent(event) {
            ...
            if (status)
              status.textContent = `正在执行：${stageLabels[event.stage]}`;
            const events = root.querySelector<HTMLElement>(
              ".trial-result__events",
            );
            if (events)
              events.appendChild(
                createElement(
                  "p",
                  "trial-result__event",
                  `${stageLabels[event.stage]} / ${event.state}`,
                ),
              );
          },
        },
  ...
  const approveTrial = async (): Promise<void> => {
    ...
    setNotice(
      "success",
      `已确认 ${state.trial.groupName} 试套合格，正式批次已解锁`,
    );
    await persist();
    render();
  };
  ...
  const retryFailed = async (): Promise<void> => {
    ...
    try {
      ...
      state.recoverableRuns = state.recoverableRuns.map((record) =>
        record.runId === state.batch?.runId
  ...
  const confirmManualCleanup = async (groupName: string): Promise<void> => {
    ...
    render();
    try {
      state.batch = await services.confirmManualCleanup(
        state.batch.runId,
        groupName,
      );
      state.recoverableRuns = state.recoverableRuns.map((record) =>
      ...
      setNotice(
        "success",
        `已记录 ${groupName} 的人工清理确认；系统已重新计算可恢复状态`,
      );
      await persist();
    } catch (error) {
  ...
  const renderTemplateStage = (container: HTMLElement): void => {
    const summary = createElement(
      "section",
      "operator-card operator-card--summary",
    );
    summary.appendChild(
      createElement(
        "p",
        "operator-card__eyebrow",
        state.templateFingerprint && !state.templateDirty
          ? "草稿已保存"
          : "等待保存",
      ),
    );
    summary.appendChild(
      createElement(
        ...
        state.template
          ? `${state.template.templateId} / ${state.template.version}`
          : "尚未登记模板",
      ),
    );
    summary.appendChild(
    ...
    const editorNote = createElement(
      "p",
      "editor-change-note",
      state.templateDirty ? "内容已修改，尚未保存。" : "",
    );
    const updateTemplateText = (): void => {
      ...
      const summaryState = root.querySelector<HTMLElement>(
        ".operator-card__eyebrow",
      );
    ...
    const select = actionButton(
      "选择模板配置",
      "secondary-action template-import",
      Boolean(state.busy),
    );
  ...
  const renderInputStage = (container: HTMLElement): void => {
    if (!state.templateFingerprint || state.templateDirty) {
      container.appendChild(
        createElement(
          "div",
          "locked-panel",
          "此阶段已锁定。请先在模板阶段保存无错误的草稿。",
        ),
      );
      return;
    }
    const editorLabel = createElement(
      "label",
      "field-label",
      "素材分组清单 JSON",
    );
    ...
    editor.placeholder =
      "选择素材总文件夹后自动生成清单；也可在诊断时粘贴清单 JSON。";
    ...
    appendChildren(container, editorLabel, editor, editorNote);
    const actions = createElement("div", "action-row");
    const scan = actionButton(
      "选择素材总文件夹",
      "secondary-action input-scan",
      Boolean(state.busy),
    );
    ...
    const preflight = actionButton(
      "预检当前清单",
      "primary-action input-preflight",
      Boolean(state.busy) || !state.groupsText.trim(),
    );
  ...
  const renderPreviewStage = (container: HTMLElement): void => {
    ...
    if (state.masterAccessRequired) {
      container.appendChild(
        createElement(
          "div",
          "locked-panel",
          "此阶段已锁定。请回到模板阶段，选择登记的母版 PSD/PSB 并核对内容指纹。",
        ),
      );
      return;
    }
    const selectorLabel = createElement(
      "label",
      "field-label",
      "选择试套素材组",
    );
    ...
    if (
      state.trialRun ||
      state.trialEvents.length > 0 ||
      state.busy === "trial"
    ) {
      const result = createElement("section", "trial-result");
      const resultStatus = !state.trialRun
        ? "running"
        : trialRunIsClean(state)
          ? "completed"
          : "failed";
      ...
      for (const event of state.trialEvents)
        events.appendChild(
          createElement(
            "p",
            "trial-result__event",
    ...
    const approve = actionButton(
      ...
      !services.capability.productionEnabled ||
        !trialRunIsClean(state) ||
        state.trial?.approved ||
        Boolean(state.busy),
    );
    approve.title = services.capability.productionEnabled
      ? "检查真实输出后确认"
      : "诊断试套不能解锁正式生产";
  ...
  const renderRunStage = (container: HTMLElement): void => {
    ...
    const checks = [
      {
        ready: Boolean(state.templateFingerprint && !state.templateDirty),
        label: "模板草稿已保存",
      },
      {
        ready: Boolean(state.report?.validGroupCount),
        label: "素材预检有可运行组",
      },
      { ready: trialIsCurrent(state), label: "当前组合试套已确认" },
      { ready: !state.masterAccessRequired, label: "母版访问仍有效" },
      {
        ready: services.capability.productionEnabled,
        label: "M0 与生产能力已验证",
      },
    ];
    for (const check of checks) {
      const row = createElement(
        "p",
        `readiness-item readiness-item--${check.ready ? "ready" : "blocked"}`,
      );
      appendChildren(
        row,
        createElement("span", "readiness-item__mark", check.ready ? "✓" : "×"),
        createElement("span", undefined, check.label),
      );
      checklist.appendChild(row);
    }
    container.appendChild(checklist);
    if (reason)
      container.appendChild(createElement("p", "action-advice", reason));
    ...
    if (state.busy === "batch" || state.busy === "retry") {
      ...
      appendChildren(
        ...
        createElement(
          "p",
          "batch-progress__eyebrow",
          state.stopRequested ? "已请求安全停止" : "批次运行中",
        ),
        createElement(
          "h3",
          "batch-progress__group",
          state.currentGroup ?? "准备下一组",
        ),
        createElement(
          "p",
          "batch-progress__stage",
          state.currentStage ? stageLabels[state.currentStage] : "正在准备",
        ),
      );
      ...
      stop.addEventListener("click", () => {
        ...
        setNotice(
          "warning",
          "已请求停止；当前操作完成后将在安全边界结束，不会提交未完成组",
        );
        render();
      });
  ...
  const renderResultsStage = (container: HTMLElement): void => {
    for (const failure of state.persistenceFailures) {
      container.appendChild(
        createElement(
          "p",
          "recovery-error",
          `运行记录 ${failure.source} 无法读取：${failure.message}`,
        ),
      );
    }
    if (!state.batch) {
    ...
    if (state.recoverableRuns.length > 1) {
      const runLabel = createElement(
        "label",
        "field-label",
        "选择要处理的中断批次",
      );
      runLabel.htmlFor = "recoverable-run-select";
      const runSelector = createElement(
        "select",
        "group-select recoverable-run-select",
      );
      ...
      runSelector.addEventListener("change", () => {
        const selected = state.recoverableRuns.find(
          (record) => record.runId === runSelector.value,
        );
        if (!selected) return;
    ...
    appendChildren(
      ...
      createElement(
        ...
        `运行 ${state.batch.runId}`,
      ),
      createElement(
        "h3",
        "result-summary__title",
        state.batch.status === "completed" ? "批次全部完成" : "批次需要处理",
      ),
      createElement(
        "p",
    ...
    for (const stateName of [
      ...
      board.appendChild(
        renderResultColumn(stateName, state.batch.groups, (groupName) => {
          void confirmManualCleanup(groupName);
        }),
      );
    }
    container.appendChild(board);
    const needsRecovery = state.batch.groups.some(
      (group) =>
        group.state === "running" ||
        group.state === "queued" ||
        group.state === "review-required" ||
    ...
    const eligible = state.batch.groups.some(
      (group) =>
        (group.state === "failed" || group.state === "interrupted") &&
    ...
    if (eligible && retryReason)
      resultActions.appendChild(
        createElement("p", "action-advice", retryReason),
      );
  ...
  function render(): void {
    ...
    const capability = createElement(
      "section",
      `capability-card capability-card--${services.capability.productionEnabled ? "ready" : "locked"}`,
    );
    ...
    if (state.notice) {
      const notice = createElement(
        "div",
        `global-notice global-notice--${state.notice.kind}`,
        state.notice.message,
      );
      notice.setAttribute(
        "role",
        state.notice.kind === "error" ? "alert" : "status",
      );
      notice.setAttribute(
        "aria-live",
        state.notice.kind === "error" ? "assertive" : "polite",
      );
      shell.appendChild(notice);
    }
    if (state.busy === "loading")
      shell.appendChild(
        createElement("div", "loading-state", "正在恢复模板草稿和中断批次…"),
      );
    ...
    for (const stage of workflowStages) {
      ...
      const button = createElement(
        "button",
        `stage-nav__item stage-nav__item--${status}`,
      );
      button.type = "button";
      ...
      button.setAttribute(
        "aria-current",
        state.viewedStage === stage.id ? "step" : "false",
      );
    ...
    const stage = workflowStages.find(
      (candidate) => candidate.id === state.viewedStage,
    )!;
    ...
    appendChildren(
      ...
      createElement(
        "span",
        "diagnostics__value",
        state.busy ? `忙碌 / ${state.busy}` : "就绪 / Manifest v5 / API v2",
      ),
    );
  ...
  const ready = (async () => {
    ...
    let recovery: Awaited<ReturnType<OperatorServices["listRecoverableRuns"]>> =
      { records: [], failures: [] };
    try {
      recovery = await services.listRecoverableRuns();
    } catch (error) {
      state.persistenceFailures.push({
        source: "运行记录目录",
    ...
    if (loaded.workspace?.draft) {
      ...
      state.templateDirty = false;
      state.masterAccessRequired = true;
      state.trial = loaded.workspace.trial
        ? { ...loaded.workspace.trial }
        : undefined;
      setNotice(
        "warning",
        "已恢复模板草稿；为避免保存会话 token，母版文件必须重新选择并核对指纹",
      );
    }
    ...
    for (const record of recovery.records) {
      try {
        ...
        state.persistenceFailures.push({
          source: record.runId,
          message: `自动恢复失败：${errorMessage(error)}`,
        });
      }
    }
